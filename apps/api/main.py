...
from apps.api.services.market_resolution_simulation import simulate_resolution
...
from apps.api.services.structural_coverage import market_needs_structural_refresh
from apps.api.ops.structural_refresh import refresh_structural_for_market
from apps.api.services.social_coverage import market_needs_social_refresh
from apps.api.services.market_alignment_intelligence import compute_market_alignment_daily
from apps.api.ops.social_refresh import refresh_social_for_market

from apps.api.services.market_ai_context_packet import build_market_ai_context_packet
from apps.api.services.market_ai_context_service import generate_market_ai_context


from apps.api.services.market_snapshot_audit import persist_market_snapshot_audit
from apps.api.services.market_uma_resolution_metadata import upsert_market_uma_resolution_metadata
from apps.api.services.market_uma_resolution_intelligence import compute_market_uma_resolution_intelligence_daily

from apps.api.services.market_design_rewrite_intelligence import evaluate_market_design_rewrite

from apps.api.services.market_oracle_linking import link_market_to_oracle
from apps.api.services.market_resolved_learning import compute_market_resolved_learning
from apps.api.services.market_resolution_patterns import compute_market_resolution_patterns
from apps.api.services.market_resolution_evaluation import evaluate_resolution_outcome

from apps.api.services.market_design_intelligence import evaluate_market_design
from apps.api.services.market_intervention_intelligence import compute_market_intervention_intelligence
from apps.api.services.market_intervention_history import persist_market_intervention_intelligence
...
def market_exists(market_id: str) -> bool:
    ...
    FROM (
        ...
        UNION
        SELECT market_id FROM public.market_uma_resolution_metadata WHERE market_id = %s
        UNION
        SELECT market_id FROM public.market_uma_resolution_intelligence_daily WHERE market_id = %s
    ) x
    LIMIT 1;
    """
    with psycopg.connect(get_db_dsn()) as conn:
        with conn.cursor() as cur:
            cur.execute(q, (market_id, market_id, market_id, market_id, market_id))
...
def should_refresh_social_for_snapshot(
    social_intelligence: dict | None,
    max_age_days: int = 1,
) -> bool:
    if not social_intelligence:
        return True

    social_day = social_intelligence.get("day")
    if not social_day:
        return True
    ...
    try:
        ...
        if isinstance(social_day, str):
            social_day_value = _dt_cls.fromisoformat(social_day).date()
        elif isinstance(social_day, _dt_cls):
            social_day_value = social_day.date()
        elif isinstance(social_day, _date_cls):
            social_day_value = social_day
        else:
            return True

        today_utc = _dt_cls.now(_tz.utc).date()
        age_days = (today_utc - social_day_value).days
...
def _delta(before, after, digits: int = 4):
...
def _flag_diff(before_flags, after_flags):
...
def build_market_score_diffs(market_id: str) -> Dict[str, Any]:
    social_q = """
    SELECT
      day::text AS day,
      demand_state,
    ...
    with psycopg.connect(get_db_dsn()) as conn:
        with conn.cursor() as cur:
            cur.execute(social_q, (market_id,))
            social_rows = rows_as_dicts(cur)

            cur.execute(alignment_q, (market_id,))
            alignment_rows = rows_as_dicts(cur)

    social_after = social_rows[0] if len(social_rows) >= 1 else {}
    social_before = social_rows[1] if len(social_rows) >= 2 else {}

    alignment_after = alignment_rows[0] if len(alignment_rows) >= 1 else {}
    alignment_before = alignment_rows[1] if len(alignment_rows) >= 2 else {}

    before = {
        "social_day": social_before.get("day"),
        "demand_state": social_before.get("demand_state"),
        "demand_score": _safe_float(social_before.get("demand_score")),
        "alignment_day": alignment_before.get("day"),
        "alignment_state": alignment_before.get("alignment_state"),
        "alignment_score": _safe_float(alignment_before.get("alignment_score")),
    }

    after = {
        "social_day": social_after.get("day"),
        "demand_state": social_after.get("demand_state"),
        "demand_score": _safe_float(social_after.get("demand_score")),
        "alignment_day": alignment_after.get("day"),
        "alignment_state": alignment_after.get("alignment_state"),
        "alignment_score": _safe_float(alignment_after.get("alignment_score")),
    }

    driver_deltas = []

    social_driver_metrics = [
    ...
    for metric in social_driver_metrics:
        b = _safe_float(social_before.get(metric))
        a = _safe_float(social_after.get(metric))
        d = _delta(b, a)
    ...
    alignment_gap_before = _safe_float(alignment_before.get("attention_vs_structure_gap"))
    alignment_gap_after = _safe_float(alignment_after.get("attention_vs_structure_gap"))
    alignment_gap_delta = _delta(alignment_gap_before, alignment_gap_after)
    if alignment_gap_delta is not None and alignment_gap_delta != 0:
        driver_deltas.append(
            {
                "metric": "attention_vs_structure_gap",
                "before": alignment_gap_before,
                "after": alignment_gap_after,
                "delta": alignment_gap_delta,
            }
        )

    deltas = {
        "demand_score_delta": _delta(
            _safe_float(social_before.get("demand_score")),
            _safe_float(social_after.get("demand_score")),
        ),
        "alignment_score_delta": _delta(
            _safe_float(alignment_before.get("alignment_score")),
            _safe_float(alignment_after.get("alignment_score")),
        ),
        "demand_state_changed": _changed(
            social_before.get("demand_state"),
            social_after.get("demand_state"),
        ),
        "alignment_state_changed": _changed(
            alignment_before.get("alignment_state"),
            alignment_after.get("alignment_state"),
        ),
        "driver_deltas": driver_deltas,
        "social_flags": _flag_diff(
            social_before.get("flags") or [],
            social_after.get("flags") or [],
        ),
        "alignment_flags": _flag_diff(
            alignment_before.get("flags") or [],
            alignment_after.get("flags") or [],
        ),
    }

    summary_parts = []

    demand_delta = deltas["demand_score_delta"]
    if demand_delta is not None and demand_delta != 0:
        direction = "rose" if demand_delta > 0 else "declined"
        summary_parts.append(f"demand score {direction} by {abs(demand_delta):.2f}")

    alignment_delta = deltas["alignment_score_delta"]
    if alignment_delta is not None and alignment_delta != 0:
        direction = "rose" if alignment_delta > 0 else "declined"
        summary_parts.append(f"alignment score {direction} by {abs(alignment_delta):.2f}")

    if (
        deltas["demand_state_changed"]
        and social_before.get("demand_state") is not None
        and social_after.get("demand_state") is not None
    ):
        summary_parts.append(
            f"demand state changed from {social_before.get('demand_state')} to {social_after.get('demand_state')}"
        )

    if (
        deltas["alignment_state_changed"]
        and alignment_before.get("alignment_state") is not None
        and alignment_after.get("alignment_state") is not None
    ):
        summary_parts.append(
            f"alignment state changed from {alignment_before.get('alignment_state')} to {alignment_after.get('alignment_state')}"
        )

    summary = "; ".join(summary_parts) + "." if summary_parts else "No score change detected."
...
def get_latest_uma_resolution_intelligence(market_id: str):
    q = """
    SELECT
        market_id,
        day,
...
class LaunchCandidateDetailResponse(BaseModel):
    ...
    speculative_flow_penalty: float
    manipulation_penalty: float
    structural_readiness_score: Optional[float] = None
    structural_integrity_score: Optional[float] = None
    structural_readiness: Optional[str] = None
...
class SocialIntelligenceRow(BaseModel):
    ...
    recommendation: str
    demand_strength_score: Optional[float] = None
    demand_breadth_score: Optional[float] = None
    demand_quality_score: Optional[float] = None
    narrative_coherence_score: Optional[float] = None
    social_fragility_score: Optional[float] = None
    summary: str
    flags: List[str] = []
    title: Optional[str] = None
    category: Optional[str] = None
...
class MarketExplorerRow(BaseModel):
    market_id: str
    question: Optional[str] = None
    category: Optional[str] = None
    url: Optional[str] = None

    structural_day: Optional[str] = None
    social_day: Optional[str] = None
    alignment_day: Optional[str] = None

    structural_score: Optional[float] = None
    structural_state: Optional[str] = None

    social_score: Optional[float] = None
    social_state: Optional[str] = None

    alignment_state: Optional[str] = None
    contextual_summary: Optional[str] = None

    participation_quality_score: Optional[float] = None
    liquidity_durability_score: Optional[float] = None
    ...
    incentive_dependency: Optional[str] = None
    activity_quality: Optional[str] = None
    intervention_intelligence: Optional[Dict[str, Any]] = None

    neutral_share: Optional[float] = None
    whale_share: Optional[float] = None
    speculative_share: Optional[float] = None

    flags: List[str] = []


class MarketDesignIntelligenceRequest(BaseModel):
    title: str
    description: Optional[str] = None
    category: Optional[str] = None
    market_type: Optional[str] = None
    protocol: Optional[str] = "polymarket"
    oracle_family: Optional[str] = "uma_oo"

class UmaResolutionMetadataRequest(BaseModel):
    payload: Dict[str, Any]


class MarketDesignRewriteIntelligenceRequest(BaseModel):
    title: str
    description: Optional[str] = None
    category: Optional[str] = None
    market_type: Optional[str] = None
...
@app.post("/ops/admin/audit/snapshot")
def admin_audit_snapshot(
    market_id: str = Query(...),
):
    snapshot = ops_market_snapshot(market_id=market_id)
    return persist_market_snapshot_audit(snapshot, source="admin_snapshot_audit")


@app.post("/ops/admin/uma/upsert")
def admin_upsert_uma_resolution_metadata(
    request: UmaResolutionMetadataRequest,
):
    return upsert_market_uma_resolution_metadata(request.payload)


@app.post("/ops/admin/compute/uma_resolution_intelligence")
def admin_compute_uma_resolution_intelligence(
    day: Optional[date] = Query(default=None),
    market_id: Optional[str] = Query(default=None),
):
    return compute_market_uma_resolution_intelligence_daily(
        day=day,
        market_id=market_id,
    )
...
class MarketExplorerResponse(BaseModel):
    day: Optional[str] = None
    rows: List[MarketExplorerRow] = []
...
@app.get("/ops/markets/{market_id}/link_oracle")
def link_oracle(market_id: str):
    return link_market_to_oracle(market_id)
...
def market_launch_intelligence(market_id: str):
    ...
    SELECT
      ...
      li.speculative_flow_penalty::float AS speculative_flow_penalty,
      li.manipulation_penalty::float AS manipulation_penalty,
      li.structural_readiness_score::float AS structural_readiness_score,
      li.structural_integrity_score::float AS structural_integrity_score,
      li.structural_readiness,
      li.structural_integrity,
      li.structure_state,
      li.recommendation,
      li.recommendation_reason,
      COALESCE(li.flags, ARRAY[]::text[]) AS flags,
...
def market_social_intelligence(market_id: str):
    ...
    SELECT
      ...
      si.demand_state,
      si.recommendation,
      si.demand_strength_score::float AS demand_strength_score,
      si.demand_breadth_score::float AS demand_breadth_score,
      si.demand_quality_score::float AS demand_quality_score,
      si.narrative_coherence_score::float AS narrative_coherence_score,
      si.social_fragility_score::float AS social_fragility_score,
      si.summary,
...
def market_alignment_intelligence(market_id: str):
    if not market_exists(market_id):
        raise HTTPException(
            status_code=404,
            detail={
    ...
    SELECT
      ...
      a.social_state,
      a.alignment_state,
      a.alignment_score::float AS alignment_score,
      a.attention_vs_structure_gap::float AS attention_vs_structure_gap,
      a.bottleneck_type,
      a.summary,
      COALESCE(a.flags, ARRAY[]::text[]) AS flags,
      a.engine_version,
            ...
            if not row:
                raise HTTPException(
                    ...
                    detail={
                        ...
                        "message": "No alignment intelligence found for market",
                        "details": {"market_id": market_id},
                    },
                )
...
def ops_social_candidates(
    ...
    allowed = {"strong", "established", "building", "limited", "absent"}
    if recommendation_normalized and recommendation_normalized not in allowed:
        raise HTTPException(
            status_code=400,
            detail={
...
def ops_market_explorer(
    ...
    SELECT COALESCE(
      %s::date,
      (SELECT MAX(day) FROM public.market_integrity_score_daily)
    )::text AS day
    """
...
WITH latest_integrity AS (
  SELECT DISTINCT ON (i.market_id)
    i.market_id,
    i.day::text AS structural_day,
    i.title,
    i.url,
    i.category,
    CASE
  WHEN i.market_quality_score IS NULL THEN NULL
  WHEN i.market_quality_score <= 1.0 THEN i.market_quality_score * 100.0
  ELSE i.market_quality_score
END::float AS structural_score,
    CASE
  WHEN i.liquidity_health_score IS NULL THEN NULL
  WHEN i.liquidity_health_score <= 1.0 THEN i.liquidity_health_score * 100.0
  ELSE i.liquidity_health_score
END::float AS liquidity_durability_score,
    CASE
  WHEN i.concentration_risk_score IS NULL THEN NULL
  WHEN i.concentration_risk_score <= 1.0 THEN i.concentration_risk_score * 100.0
  ELSE i.concentration_risk_score
END::float AS concentration_risk_score,
    i.integrity_band,
    i.review_priority,
    i.trades,
    i.unique_traders,
    ...
    i.neutral_role_share::float AS neutral_share,
    i.whale_role_share::float AS whale_share,
    i.speculator_role_share::float AS speculative_share
  FROM public.market_integrity_score_daily i
  WHERE (%s::date IS NULL OR i.day = %s::date)
...
latest_social AS (
  SELECT DISTINCT ON (s.market_id)
    s.market_id,
    s.day::text AS social_day,
    s.demand_score::float AS social_score,
    s.demand_state AS social_state,
    s.summary AS social_summary,
    COALESCE(s.flags, ARRAY[]::text[]) AS social_flags
  FROM public.market_social_intelligence_daily s
  WHERE s.engine_version = 'market_social_intelligence_v4_mvp2'
    AND (%s::date IS NULL OR s.day = %s::date)
  ORDER BY s.market_id, s.day DESC, s.updated_at DESC
),
...
latest_alignment AS (
  SELECT DISTINCT ON (a.market_id)
    ...
    a.structural_day::text AS alignment_structural_day,
    a.social_day::text AS alignment_social_day,
    a.structural_state,
    a.social_state,
    a.alignment_state,
    a.alignment_score::float AS alignment_score,
  ...
  FROM public.market_alignment_daily a
  WHERE a.engine_version = 'market_alignment_v9_integrity_source'
    AND (%s::date IS NULL OR a.day = %s::date)
  ORDER BY a.market_id, a.day DESC, a.updated_at DESC
),

latest_intervention AS (
  SELECT DISTINCT ON (x.market_id)
    x.market_id,
    x.intervention_needed,
    x.recommended_intervention,
    x.recommended_action,
    x.action_priority,
    x.action_reason,
    x.incentive_dependency,
    x.activity_quality,
    x.organic_participation_ratio,
    x.distortion_risk,
    x.expected_failure_mode,
    x.intervention_effectiveness_estimate,
    x.confidence
  FROM public.market_intervention_intelligence_daily x
  ORDER BY x.market_id, x.day DESC, x.updated_at DESC
)

SELECT
  COALESCE(i.market_id, a.market_id, s.market_id) AS market_id,
  COALESCE(i.title, COALESCE(i.market_id, a.market_id, s.market_id)) AS question,
  i.category,
  i.url,

  COALESCE(a.alignment_structural_day, i.structural_day) AS structural_day,
  COALESCE(a.alignment_social_day, s.social_day) AS social_day,
  a.alignment_day,
  ...
  s.social_score,
  COALESCE(a.social_state, s.social_state) AS social_state,

  a.alignment_state,
  COALESCE(a.alignment_summary, s.social_summary) AS contextual_summary,

  NULL::float AS participation_quality_score,
  i.liquidity_durability_score,
  i.concentration_risk_score AS concentration_hhi,
  ...
  x.activity_quality,
  jsonb_build_object(
    'recommended_intervention', x.recommended_intervention,
    'recommended_action', x.recommended_action,
    'action_priority', x.action_priority,
    'action_reason', x.action_reason,
    'organic_participation_ratio', x.organic_participation_ratio,
    'distortion_risk', x.distortion_risk,
    'expected_failure_mode', x.expected_failure_mode,
    'intervention_effectiveness_estimate', x.intervention_effectiveness_estimate,
    'confidence', x.confidence
  ) AS intervention_intelligence,
  i.neutral_share,
  i.whale_share,
  i.speculative_share,
  ...
  ARRAY(
    ...
    FROM unnest(
      COALESCE(a.alignment_flags, ARRAY[]::text[]) ||
      COALESCE(s.social_flags, ARRAY[]::text[])
    ) AS flag
  ) AS flags

FROM latest_integrity i
FULL OUTER JOIN latest_alignment a
  ON a.market_id = i.market_id
FULL OUTER JOIN latest_social s
  ON s.market_id = COALESCE(i.market_id, a.market_id)
LEFT JOIN latest_intervention x
  ON x.market_id = COALESCE(i.market_id, a.market_id, s.market_id)

ORDER BY
  CASE COALESCE(a.alignment_state, 'weak')
    WHEN 'confirmed' THEN 1
    WHEN 'conviction_mismatch' THEN 2
    WHEN 'structure_led' THEN 3
    ELSE 4
  END,
  COALESCE(i.structural_score, 0) DESC,
  COALESCE(s.social_score, 0) DESC,
  COALESCE(i.market_id, a.market_id, s.market_id)
LIMIT %s;
"""
    ...
    with psycopg.connect(get_db_dsn()) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(q, (day, day, day, day, day, day, limit))
            rows = rows_as_dicts(cur)
...
def ops_market_snapshot(
    ...
    resolution_intelligence = safe_call(
        "resolution_intelligence",
        lambda: get_latest_uma_resolution_intelligence(market_id),
        {},
    )

    social_refresh_result = None
    needs_social_refresh = should_refresh_social_for_snapshot(
        social_intelligence,
        max_age_days=1,
    )
    ...
    if not alignment_intelligence and social_intelligence:
        structural_score = (
            market.get("market_quality_score")
            if isinstance(market, dict)
            else None
        )

        if structural_score is not None and structural_score >= 0.8:
            fallback_structural_state = "strong"
        elif structural_score is not None and structural_score >= 0.6:
            fallback_structural_state = "moderate"
        else:
            fallback_structural_state = "weak"

        fallback_social_state = social_intelligence.get("demand_state") or "absent"

        if fallback_structural_state == "strong" and fallback_social_state in ("strong", "established"):
            fallback_alignment_state = "confirmed"
            fallback_summary = "Attention appears to be supported by credible participation."
            fallback_bottleneck = None
        elif fallback_structural_state in ("strong", "moderate") and fallback_social_state == "absent":
            fallback_alignment_state = "structure_led"
            fallback_summary = "Structure appears healthier than current demand activation."
            fallback_bottleneck = "demand"
        elif fallback_social_state in ("strong", "established", "building"):
            fallback_alignment_state = "conviction_mismatch"
            fallback_summary = "Demand appears ahead of participation quality."
            fallback_bottleneck = "structure"
        else:
            fallback_alignment_state = "weak"
            fallback_summary = "Attention and participation both remain limited."
            fallback_bottleneck = "structure"

        alignment_intelligence = {
            "market_id": market_id,
            "day": social_intelligence.get("day"),
            "structural_state": fallback_structural_state,
            "social_state": fallback_social_state,
            "alignment_state": fallback_alignment_state,
            "alignment_score": None,
            "attention_vs_structure_gap": None,
            "bottleneck_type": fallback_bottleneck,
            "summary": fallback_summary,
            "flags": ["STRUCTURAL_COVERAGE_MISSING"],
            "engine_version": "snapshot_fallback_alignment_v5",
        }
    ...
    resolution_only_mode = (
        bool(resolution_intelligence)
        and not bool(market)
        and not bool(social_intelligence)
        and not bool(alignment_intelligence)
    )
    ...
    anchor_day = (impact or {}).get("anchor_day")

    structural_day = (market or {}).get("structural_day") or (market or {}).get("day")
    social_day = (social_intelligence or {}).get("day")
    alignment_day = (alignment_intelligence or {}).get("day")
    ...
    is_mixed_horizon = bool(
        ...
        and alignment_day
        and (
            str(structural_day) != str(social_day)
            or str(structural_day) != str(alignment_day)
        )
    )
    effective_structural_score = (
        market.get("market_quality_score")
        if isinstance(market, dict)
        else None
    )

    if effective_structural_score is not None and effective_structural_score <= 1.0:
        effective_structural_score = float(effective_structural_score) * 100.0

    alignment_structural_state = (
        alignment_intelligence.get("structural_state")
        if isinstance(alignment_intelligence, dict)
        else None
    )

    effective_structural_state = (
        alignment_structural_state
        or (
            "strong"
            if effective_structural_score is not None and effective_structural_score >= 80
            else (
                "moderate"
                if effective_structural_score is not None and effective_structural_score >= 60
                else (
                    "weak"
                    if effective_structural_score is not None
                    else (
                        (market.get("structural_state") if isinstance(market, dict) else None)
                        or "weak"
                    )
                )
            )
    ...
    effective_social_state = (
        (alignment_intelligence.get("social_state") if isinstance(alignment_intelligence, dict) else None)
        or (social_intelligence.get("demand_state") if isinstance(social_intelligence, dict) else None)
        or (social_intelligence.get("social_state") if isinstance(social_intelligence, dict) else None)
        or "absent"
    )

    effective_alignment_state = (
        (alignment_intelligence.get("alignment_state") if isinstance(alignment_intelligence, dict) else None)
    )

    if not isinstance(alignment_intelligence, dict):
        alignment_intelligence = {}

    if not resolution_only_mode:
        alignment_intelligence["structural_state"] = effective_structural_state
        alignment_intelligence["social_state"] = effective_social_state
        alignment_intelligence["alignment_state"] = (
            effective_alignment_state or "weak"
        )


    ai_context = {}
    try:
        should_compute_ai_context = (
            (
                isinstance(alignment_intelligence, dict)
                and bool(alignment_intelligence.get("alignment_state"))
            )
            or bool(resolution_intelligence)
        )

        if should_compute_ai_context:
            packet, packet_hash, drivers = build_market_ai_context_packet(
                market,
                social_intelligence,
                alignment_intelligence,
                resolution_intelligence,
                impact,
                {
                    ...
                    "resolution_day": (resolution_intelligence or {}).get("day"),
                    "social_refresh": social_refresh_result,
                    "is_mixed_horizon": is_mixed_horizon,
                },
            )

            if isinstance(alignment_intelligence, dict) and alignment_intelligence:
                alignment_intelligence["drivers"] = drivers
                ...
                alignment_intelligence["neutral_share"] = float(
                    (recent_map.get("NEUTRAL") or {}).get("notional_share") or 0.0
                )
                alignment_intelligence["whale_share"] = float(
                    (recent_map.get("WHALE") or {}).get("notional_share") or 0.0
                )
                alignment_intelligence["speculative_share"] = float(
                    (recent_map.get("SPECULATOR") or {}).get("notional_share") or 0.0
                )

            ai_context = generate_market_ai_context(
                packet,
                packet_hash,
                drivers,
                None,
...
"social_intelligence": (
    ...
    if resolution_only_mode
        ...
        **(social_intelligence or {}),
        "demand_score": (
            (float((social_intelligence or {}).get("demand_score")) / 100.0)
            if (social_intelligence or {}).get("demand_score") is not None
            and float((social_intelligence or {}).get("demand_score")) > 1.0
            else (social_intelligence or {}).get("demand_score")
        ),
        "demand_state": (
            (social_intelligence or {}).get("demand_state")
            or (social_intelligence or {}).get("social_state")
            or "absent"
        ),
        "demand_strength_score": (
            (float((social_intelligence or {}).get("demand_strength_score")) / 100.0)
            if (social_intelligence or {}).get("demand_strength_score") is not None
            and float((social_intelligence or {}).get("demand_strength_score")) > 1.0
            else (social_intelligence or {}).get("demand_strength_score")
        ),
        "demand_breadth_score": (
            (float((social_intelligence or {}).get("demand_breadth_score")) / 100.0)
            if (social_intelligence or {}).get("demand_breadth_score") is not None
            and float((social_intelligence or {}).get("demand_breadth_score")) > 1.0
            else (social_intelligence or {}).get("demand_breadth_score")
        ),
        "demand_quality_score": (
            (float((social_intelligence or {}).get("demand_quality_score")) / 100.0)
            if (social_intelligence or {}).get("demand_quality_score") is not None
            and float((social_intelligence or {}).get("demand_quality_score")) > 1.0
            else (social_intelligence or {}).get("demand_quality_score")
        ),
        "narrative_coherence_score": (
            (float((social_intelligence or {}).get("narrative_coherence_score")) / 100.0)
            if (social_intelligence or {}).get("narrative_coherence_score") is not None
            and float((social_intelligence or {}).get("narrative_coherence_score")) > 1.0
            else (social_intelligence or {}).get("narrative_coherence_score")
        ),
        "social_fragility_score": (
            (float((social_intelligence or {}).get("social_fragility_score")) / 100.0)
            if (social_intelligence or {}).get("social_fragility_score") is not None
            and float((social_intelligence or {}).get("social_fragility_score")) > 1.0
            else (social_intelligence or {}).get("social_fragility_score")
        ),
        "confidence_score": (
            (float((social_intelligence or {}).get("confidence_score")) / 100.0)
            if (social_intelligence or {}).get("confidence_score") is not None
            and float((social_intelligence or {}).get("confidence_score")) > 1.0
            else (social_intelligence or {}).get("confidence_score")
        ),
        "horizon": "latest_social_day",
        "social_day": social_day,
...
"alignment_intelligence": (
    ...
    if resolution_only_mode
        ...
        **(alignment_intelligence or {}),
        "horizon": "latest_alignment_day",
        "alignment_day": alignment_day,
        "structural_day": structural_day,
        "social_day": social_day,
        "is_mixed_horizon": is_mixed_horizon,
    } if alignment_intelligence else {}
),

        "resolution_intelligence": {
            **(resolution_intelligence or {}),
            "horizon": "latest_resolution_day",
            "resolution_day": (resolution_intelligence or {}).get("day"),
        } if resolution_intelligence else {},
        ...
        "score_diffs": score_diffs or {},
        "errors": errors,

        "snapshot_meta": {
            "market_horizon": "latest_structural_day",
            "social_horizon": "latest_social_day",
            "alignment_horizon": "latest_alignment_day",
            "is_mixed_horizon": is_mixed_horizon,
            "structural_day": structural_day,
            "social_day": social_day,
            "alignment_day": alignment_day,
            ...
            "interpretation": {
                "market": "Latest available structural day for this market.",
                "social_intelligence": "Latest demand and attention signal for the social day used for this market.",
                "alignment_intelligence": "Latest alignment view for the alignment day used for this market. Structural and social inputs may come from different horizons.",
                "resolution_intelligence": "Latest available UMA resolution risk and clarity view for this market.",
                "traders": {
    ...
    snapshot["market_id"] = market_block.get("market_id") or market_id
    snapshot["question"] = (
        market_block.get("question")
        or market_block.get("title")
        or snapshot["market_id"]
    )

    structural_score = market_block.get("market_quality_score")
    if structural_score is not None and structural_score > 1.0:
        structural_score = float(structural_score) / 100.0

    social_score = (
        social_intelligence.get("demand_score")
        if social_intelligence.get("demand_score") is not None
        else None
    )
    if social_score is not None and social_score > 1.0:
        social_score = float(social_score) / 100.0

    if resolution_only_mode:
        snapshot["structural_score"] = None
        snapshot["social_score"] = None
        snapshot["structural_state"] = None
        snapshot["social_state"] = None
        snapshot["alignment_state"] = None
        snapshot["integrity_band"] = None
    else:
        snapshot["structural_score"] = structural_score
        snapshot["social_score"] = social_score

        snapshot["structural_state"] = (
            alignment_intelligence.get("structural_state")
        ...
        snapshot["social_state"] = (
            ...
            or alignment_intelligence.get("social_state")
            or "absent"
        )

        snapshot["alignment_state"] = (
            alignment_intelligence.get("alignment_state")
            or "weak"
        )

        snapshot["integrity_band"] = market_block.get("integrity_band") or "unknown"

    snapshot["resolution_state"] = (
        (resolution_intelligence or {}).get("resolution_state")
    )
    snapshot["resolution_clarity_score"] = (
        (resolution_intelligence or {}).get("resolution_clarity_score")
    )
    snapshot["oracle_risk_score"] = (
        (resolution_intelligence or {}).get("oracle_risk_score")
    )

    intervention_intelligence = compute_market_intervention_intelligence(snapshot)
    snapshot["intervention_intelligence"] = intervention_intelligence
    snapshot["intervention_needed"] = intervention_intelligence.get("intervention_needed")
    snapshot["incentive_dependency"] = intervention_intelligence.get("incentive_dependency")
    snapshot["activity_quality"] = intervention_intelligence.get("activity_quality")
    snapshot["recommended_action"] = intervention_intelligence.get("recommended_action")
    snapshot["action_priority"] = intervention_intelligence.get("action_priority")
    snapshot["action_reason"] = intervention_intelligence.get("action_reason")

    try:
        snapshot["intervention_persistence"] = persist_market_intervention_intelligence(
            market_id=market_id,
            snapshot=snapshot,
    ...
    # -----------------------------
    if resolution_only_mode:
        snapshot["contextual_summary"] = (
            (resolution_intelligence or {}).get("summary")
            or "Resolution intelligence is available, but live market-quality coverage is not available for this market."
        )
    else:
        snapshot["contextual_summary"] = (
            ai_context.get("contextual_summary") if ai_context else None
        )

    snapshot["audit_id"] = ai_context.get("audit_id") if ai_context else None
    snapshot["cache_hit"] = ai_context.get("cache_hit") if ai_context else None

    if ai_context:
        snapshot["ai_context"] = {
            "interpretation": ai_context.get("interpretation"),
            "interpretation_type": ai_context.get("interpretation_type"),
            "confidence": ai_context.get("confidence"),
            "does_demand_reflect_conviction": ai_context.get("does_demand_reflect_conviction"),
            "does_alignment_imply_convergence": ai_context.get("does_alignment_imply_convergence"),
            "drivers": ai_context.get("drivers") or [],
            "caution_flags": ai_context.get("caution_flags") or [],
            "abstain": ai_context.get("abstain"),
            "abstain_reason": ai_context.get("abstain_reason"),
            "model_provider": ai_context.get("model_provider"),
            "model_name": ai_context.get("model_name"),
        }
    else:
        snapshot["ai_context"] = None

    snapshot["coverage_summary"] = build_coverage_summary(snapshot) or {}

    has_social_intelligence = bool(
        snapshot.get("social_intelligence")
        and (
            snapshot["social_intelligence"].get("demand_score") is not None
            or snapshot["social_intelligence"].get("demand_state") is not None
        )
    )

    has_alignment_intelligence = bool(
        snapshot.get("alignment_intelligence")
        and (
            snapshot["alignment_intelligence"].get("alignment_state") is not None
            or snapshot["alignment_intelligence"].get("alignment_score") is not None
        )
    )

    snapshot["coverage_summary"]["has_social_intelligence"] = has_social_intelligence
    snapshot["coverage_summary"]["has_alignment_intelligence"] = has_alignment_intelligence
    snapshot["coverage_summary"]["has_contextual_summary"] = bool(snapshot.get("contextual_summary"))
    snapshot["coverage_summary"]["has_resolution_intelligence"] = bool(snapshot.get("resolution_intelligence"))
    snapshot["coverage_summary"]["resolution_only_mode"] = resolution_only_mode

    if resolution_only_mode:
        snapshot["coverage_summary"]["coverage_level"] = "resolution_only"
        snapshot["coverage_summary"]["coverage_reason"] = (
            "Resolution intelligence is available, but structural, demand, and alignment coverage are not available for this market."
        )
    ...
    response = {
        ...
        "structural_score": snapshot.get("structural_score"),
        "social_score": snapshot.get("social_score"),

        "structural_state": snapshot.get("structural_state"),
        "social_state": snapshot.get("social_state"),
        "alignment_state": snapshot.get("alignment_state"),
        "integrity_band": snapshot.get("integrity_band"),
        "resolution_state": snapshot.get("resolution_state"),

        "contextual_summary": snapshot.get("contextual_summary"),

        "intervention_needed": snapshot.get("intervention_needed"),
        "recommended_action": snapshot.get("recommended_action"),
        "action_priority": snapshot.get("action_priority"),
        "action_reason": snapshot.get("action_reason"),
        "incentive_dependency": snapshot.get("incentive_dependency"),
        "activity_quality": snapshot.get("activity_quality"),
        "intervention_intelligence": snapshot.get("intervention_intelligence"),
        "resolution_clarity_score": snapshot.get("resolution_clarity_score"),
        "oracle_risk_score": snapshot.get("oracle_risk_score"),
        ...
        "is_mixed_horizon": snapshot.get("is_mixed_horizon"),
        ...
        "social_intelligence": snapshot.get("social_intelligence") or {},
        "alignment_intelligence": snapshot.get("alignment_intelligence") or {},
        "resolution_intelligence": snapshot.get("resolution_intelligence") or {},
        "traders": snapshot.get("traders") or {},
    }

    try:
        persist_market_snapshot_audit(response, source="snapshot_endpoint")
    except Exception:
        pass
...
@app.post("/ops/admin/compute/market_social_features")
def admin_compute_market_social_features(
    day: Optional[date] = Query(default=None),
    market_id: Optional[str] = Query(default=None),
    lookback_days: int = Query(default=7, ge=1, le=30),
):
    return compute_market_social_features_daily(
        day=day,
        market_id=market_id,
...
@app.post("/ops/admin/score/market_social_features")
def admin_score_market_social_features(
    day: Optional[date] = Query(default=None),
    market_id: Optional[str] = Query(default=None),
):
    return score_market_social_features_daily(
        day=day,
        market_id=market_id,
    )
...
@app.post("/ops/admin/compute/social")
def run_social_intelligence(
    market_id: str | None = None,
):
    result = compute_market_social_intelligence_daily(market_id=market_id)
    return {"status": "ok", "result": result}

@app.post("/ops/admin/compute/alignment")
def run_alignment_intelligence(
    market_id: str | None = None,
...
@app.post("/ops/admin/refresh/social")
def admin_refresh_social_for_market(
    market_id: str,
):
    return refresh_social_for_market(market_id=market_id)

@app.post("/ops/admin/compute/resolved_learning")
def admin_compute_resolved_learning(
    market_id: str = Query(...),
):
    return compute_market_resolved_learning(market_id)

@app.post("/ops/admin/compute/resolution_patterns")
def admin_compute_resolution_patterns(
    day: Optional[str] = Query(default=None),
):
    return compute_market_resolution_patterns(day=day)

@app.post("/ops/design/intelligence")
def ops_design_intelligence(
    request: MarketDesignIntelligenceRequest,
):
    return evaluate_market_design(request.model_dump())

@app.post("/ops/design/rewrite_intelligence")
def ops_design_rewrite_intelligence(
    request: MarketDesignRewriteIntelligenceRequest,
):
    return evaluate_market_design_rewrite(request.model_dump())

@app.post("/ops/design/resolution_simulation")
def resolution_simulation(payload: dict = Body(...)):
    """
    Simulate how a market will resolve before launch.
    """
    return simulate_resolution(payload)

@app.post("/ops/resolution/evaluate")
def resolution_evaluate(payload: dict = Body(...)):
    """
    Compare a prior simulation run against actual resolution behavior.
    """
    try:
        return evaluate_resolution_outcome(payload)

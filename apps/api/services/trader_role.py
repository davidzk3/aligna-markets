...
classified as (
    select
        ...
        case
            -- 🔴 Dominant whale (very high concentration risk)
            when tb.is_large_participant
                 and coalesce(tb.market_volume_share, 0) >= 0.50
                then 'dominant_whale'

            -- 🟠 Large but not dominant (normal big participant)
            when tb.is_large_participant
                 and coalesce(tb.market_volume_share, 0) between 0.20 and 0.50
                then 'large_influential'

            -- 🟡 Coordinated / farming-like behavior (high freq + directional)
            when tb.is_high_frequency
                 and tb.is_one_sided
                 and coalesce(tb.market_volume_share, 0) < 0.10
                then 'coordinated_actor'

            -- 🟡 Active trader with meaningful footprint
            when tb.is_high_frequency
                 and coalesce(tb.market_volume_share, 0) >= 0.10
                then 'active_speculator'

            -- 🟢 Maker-like (balanced + persistent)
            when tb.trades >= 3
            ...
            -- 🟠 Directional trader with size
            when tb.is_one_sided
                 and coalesce(tb.market_volume_share, 0) >= 0.10
                then 'directional_speculator'
        ...
        array_remove(array[
            ...
            case when coalesce(tb.market_volume_share, 0) >= 0.50 then 'extreme_concentration' end,
            case when coalesce(tb.market_volume_share, 0) between 0.20 and 0.50 then 'high_concentration' end,

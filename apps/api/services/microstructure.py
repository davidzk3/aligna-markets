...
active_bbo_markets as (
  ...
  from public.market_bbo_ticks b
  where b.ts >= %(start_ts)s
    and b.ts < %(end_ts)s
    and (%(market_id)s::text is null or b.market_id = %(market_id)s::text)
),
active_markets as (
...
insert into public.market_microstructure_daily (
  market_id,
  day,
  window_hours,
  volume,
  ...
  flags = excluded.flags;
    ...
    with psycopg.connect(get_db_dsn()) as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                create table if not exists public.market_microstructure_daily (
                    ...
                    top1_trader_share numeric(18,8) null,
                    top5_trader_share numeric(18,8) null,
                    hhi numeric(18,8) null,

                    price_volatility numeric(18,8) null,
                    bbo_ticks integer null,
                    avg_spread numeric(18,8) null,

                    suspicious_burst_flag boolean not null default false,
                    burst_score double precision null,

                    identity_coverage double precision null,
                    identity_blind boolean not null default false,

                    structural_score numeric(18,8) null,
            ...
            cur.execute(
                """
                alter table public.market_microstructure_daily
            ...
            cur.execute(
                """
                alter table public.market_microstructure_daily
                add column if not exists suspicious_burst_flag boolean not null default false;
                """
            )
            ...
            cur.execute(
                """
                alter table public.market_microstructure_daily
                add column if not exists burst_score double precision null;
                """
            )
            ...
            cur.execute(
                """
                alter table public.market_microstructure_daily
                add column if not exists identity_coverage double precision null;
                """
            )
            ...
            cur.execute(
                """
                alter table public.market_microstructure_daily
                add column if not exists identity_blind boolean not null default false;
                """
            )

            cur.execute(
                """
                create index if not exists idx_market_microstructure_daily_day
                on public.market_microstructure_daily (day desc);
                """
            )

            cur.execute(
                """
                create index if not exists idx_market_microstructure_daily_market_day
                on public.market_microstructure_daily (market_id, day desc);
                """
            )

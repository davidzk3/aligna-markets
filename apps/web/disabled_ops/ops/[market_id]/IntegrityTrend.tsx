...
import { fetchIntegrityHistory } from "../../../lib/api";
...
function summaryAlert(point: Point | null) {
  ...
  const integrity =
    typeof point.integrity_score === "number" ? point.integrity_score : NaN;
  const radar =
    typeof point.radar_risk_score === "number" ? point.radar_risk_score : NaN;
  const manipulation =
    typeof point.manipulation_score === "number"
      ? point.manipulation_score
      : NaN;
  const regime = String(point.regime ?? "").toLowerCase();

  if (
    (Number.isFinite(manipulation) && manipulation >= 0.7) ||
    (Number.isFinite(radar) && radar >= 70)
  ) {
    return {
